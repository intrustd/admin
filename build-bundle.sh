...
nix-build ../../kite/nix/build-bundle.nix --argstr kite-app-module "`pwd`/kite.nix" -o result.json --arg systems 'let pkgs = import <nixpkgs> {}; in builtins.listToAttrs [ { name = pkgs.hostPlatform.config; value = pkgs; } { name = "x86_64-unknown-linux-musl"; value=pkgs;}]'
